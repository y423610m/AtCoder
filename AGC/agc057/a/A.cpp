...
void solve() {
	...
	V<ll> pow10(1,1);
	rep(i,9) pow10.push_back(pow10.back()*10);
	REVERSE(pow10);
	...
	while(L+1<R){
		...
		if(mid*10<=r) ok = false;
		if(POW(10, ((ll)to_string(mid).size())) + mid <=r) ok = false;
	...
	chmax(l, r/10);
	chmax(l, stoi(to_string(r).substr(1)));
	//if(r != 10LL*((ll)to_string(r).size()-1)) chmax(l, 10LL*((ll)to_string(r).size()-1));
	if(to_string(r)[0]!='1') chmax(l, 10LL*((ll)to_string(r).size()-1)-1);
	PL(r-l)
	...
	for(auto P: pow10){
		if(l<=P && P<=r){
			...
			if(P<=r && r<P*10) ans += r-max(P,l)+1;
			
			if( P/10<=l && l<P ){
				ll L = max(l,P/10)-1; ll R = P;
				while(L+1<R){
					ll mid = (L+R)/2;
					bool ok = true;
					rep(i,10){
						if(10*mid+i<=r) ok = false;
					}
					repi(i,1,10){
						if(mid+i*P<=r) ok = false;
					}
