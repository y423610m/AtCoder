...
void solve() {
    ...
    map<char,V<int>> mp;
    rep(i,N) mp[S[i][0]].push_back(i);

    Trie<26, 'A'> tr;
    tr.insert(T);

    int p = 0;
    V<ll> dp(T.size()+1, LINF);
    dp[0] = 0;
    rep(i,T.size()){
        if(dp[i]!=LINF){
            char c = T[i];
            for(auto id:mp[c]){
                int res = tr.find(S[id], p);
                EL(res)
                if(i+S[id].size()<tr[res].depth) continue;
                if(tr[res].shared+tr[res].accept.size()){
                    EL(1)
                    EL(S[id])
                    for(ll j=i+1;j<i+S[id].size();j++){
                        if(j>=T.size()) break;
                        ES(j)
                        chmin(dp[j], dp[i]+1);
                    }
                    if(i+S[id].size()==T.size()){
                        ES(0)
                        chmin(dp[T.size()], dp[i]+1);
                        EL(2)
                    }
                    ES(tr.create_string(p)) EL(S[id])
                }
            }
        }
        p = tr[p].children[T[i]-'A'];
    }
    PL(dp[T.size()])
    EL(dp)
