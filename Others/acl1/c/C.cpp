...
void solve() {
   ...
   mcf_graph<ll,ll> mcf(N*M*2+2);//cap,cost
   ll s = N*M*2;
   ll t = N*M*2+1;

   rep(h,N) rep(w,M) if(S[h][w]=='o') mcf.add_edge(s, h*M+w, 1, 0);
   rep(h,N) rep(w,M) if(S[h][w]!='#') mcf.add_edge(h*M+w+N*M, t, 1, 0);

   rep(sh, N) rep(sw,M) if(S[sh][sw]=='o'){
      V<V<bool>> dp(N, V<bool>(M));
      dp[sh][sw] = true;
      mcf.add_edge(sh*M+sw, sh*M+sw+N*M, 1, INF);

      rep(h,N) rep(w, M) if(dp[h][w]){
         rep(d,2){
            ll nh = h+dx[d];
            ll nw = w+dy[d];
            if(inGrid(nh, nw) && S[nh][nw]!='#'){
               dp[nh][nw] = true;
               mcf.add_edge(sh*M+sw, nh*M+nw+N*M, 1, INF-(nh-sh+nw-sw));
            }
         }
      }
   }
