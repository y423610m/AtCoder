...
void solve() {
   ...
   ll N; cin>>N;
   ll Q; cin>>Q;
   Edges<ll> E = readE<ll>(N-1, 0, false);//weighted?
   Graph<ll> G(N, E, false);//directed?
   EulerTour<ll> et(N, G, 0, false, false);//root, mid, out
   EL(et.in)
   EL(et.out)

   Lazy::LazyRSQ<ll> seg(et.size());
   int p = 0;
   rep(q,Q){
      ll t; cin>>t;
      if(t==0){
         ...
         cin>>i>>k;
         // i--;
         EL(k/(1+et.numOfChildren[i]))
         seg.apply(et.in[i], et.out[i]+1, {1, k/(1+et.numOfChildren[i])});
      }
      else{
         ll i;
         cin>>i;
         int l = i, r = p;
         if(et.in[l]>et.in[r]) swap(l,r);
         int lca = et.lca(p,i);
         ll ans = 0;
         //ans = [0,lca]-[l,lca] +  [0,]-[]
         ans += seg.prod(0, et.in[r]+1).x - seg.prod(0, et.in[lca]).x;
         ans += seg.prod(0, et.in[lca]+1).x - seg.prod(0, et.in[l]).x;
         PL(ans)
         p = i;
      }
   }
