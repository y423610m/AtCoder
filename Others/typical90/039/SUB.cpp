...
void solve() {
    ...
    //iからの距離の合計
    V<ll> dp(N);
    rep(i,N) dp[0] += rt.depth[i];

    auto dfs = [&](auto dfs, int p)-> void {
        dp[p] = dp[rt.parent[p]] - (rt.numOfChildren[p]+1) + (N-(rt.numOfChildren[p]+1));
        for(const auto& e:G[p]) if(e.to!=rt.parent[p]) dfs(dfs, e.to);
    };

    for(const auto& e:G[0]) dfs(dfs, e.to);
