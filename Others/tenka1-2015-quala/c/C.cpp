...
void solve() {
   ...
   ll M,N; cin>>M>>N;
   V<V<ll>> A(M, V<ll>(N)); cin>>A;
   V<V<ll>> B(M, V<ll>(N)); cin>>B;
   ...
   rep(h,M) rep(w,N){
      if(A[h][w]==0) mf.add_edge(s,h*N+w,1);
      else mf.add_edge(h*N+w+M*N, t, 1);
   }

   rep(h,M) rep(w,N) if(A[h][w]==0&&B[h][w]==1) {
      rep(d,4){
         ll nx = h + dx[d];
         ll ny = w + dy[d];
         if(inGrid(nx, ny) && A[nx][ny]==1 && B[nx][ny] == 0){
            mf.add_edge(h*N+w, M*N+nx*N+ny, 1);
            ES(nx) EL(ny)
         }
      }
   }
   ...
   ll ans = 0;
   rep(h,M) rep(w,N) ans += (A[h][w] != B[h][w]);
   EL(ans)
   ans -= mf.flow(s, t);
   PL(ans)
