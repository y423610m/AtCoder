...
void solve() {
    ...
    Lazy::LazyRSQ<ll> X(H*W);
    rep(h,H) rep(w,W) X.set(w*H+h, {h,1});

    Lazy::LazyRSQ<ll> Y(H*W);
    rep(h,H) rep(w,W) Y.set(h*W+w, {w,1}); 
    ...
    rep(_,Q){
        ...
        a--,b--;
        rep(h,H){
            Y.apply(h*W+0,  h*W+b+1,Lazy::F<ll>{-1,b});
            Y.apply(h*W+b+1,h*W+W,Lazy::F<ll>{-1,W+b});
        }
        rep(w,W){
            // ES(w*H+0) ES(w*H+b+1) ES(w*H+b+1) EL(w*H+H)
            X.apply(w*H+0,  w*H+a+1,Lazy::F<ll>{-1,a});
            X.apply(w*H+a+1,w*H+H,Lazy::F<ll>{-1,H+a});
        }
        rep(h,H) rep(w,W){
            ES(h) EL(w) ES(X.prod(w*H+h,w*H+h+1).x) EL(Y.prod(h*W+w,h*W+w+1).x)
            //B[h][w] = A[X.prod(w*H+h,w*H+h+1).x][Y.prod(h*W+w,h*W+w+1).x];
        }
    }
