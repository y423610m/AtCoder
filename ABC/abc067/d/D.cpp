...
void solve() {
   ...
   Dijkstra<int> di(N, INF);//頂点数，初期距離
   di.solve(G, 0, -1, false);//G, start, goal, 通り数える

   auto path = di.find_path(N-1);

   V<ll> used(N, -1);
   used[0] = 0;
   used[N-1] = 1;
   ll l = 0; ll r = path.size()-1;
   rep(i,N){
      if(l+1<r){
         l++;
         used[path[l]] = 0;
   ...
   Dijkstra<int> dF(N, INF);//頂点数，初期距離
   dF.solve(G, l, -1, false);//G, start, goal, 通り数える

   Dijkstra<int> dS(N, INF);//頂点数，初期距離
   dS.solve(G, r, -1, false);//G, start, goal, 通り数える

   rep(i,N) if(used[i]==-1){
      if(dF.dist[i] < dS.dist[i]) used[i] = 0;
      else used[i] = 1;
   }

   map<ll,ll> mp;
   rep(i,N) mp[used[i]]++;
   if(mp[0]<=mp[1]) PL("Snuke")
   else PL("Fennec")
