...
void solve() {
    ...
    ll ans = 0;
    // rep(i,N){//for takahashi
    //     rep(j,N){//for aoki
    //         if(AB[i].fi<AB[j].fi) continue;
    //         if(AB[i].se>AB[j].se) continue;
    //         ans++;
    //     }
    // }
    ...
    rep(i,N){
        ES(i)
        tree.set(AB[i].se, tree.get(AB[i].se)+1);
        //pastB.push_back(AB[i].se);
        int cnt = 0;
        cnt = tree.prod(AB[i].se, N+2);
        // rep(j,pastB.size()){
        //     if(pastB[j]>=AB[i].se) cnt++;
        // }
        EL(cnt)
        ans += cnt;
        ans += mp[AB[i]];
