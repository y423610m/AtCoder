...
void solve() {
   ...
   V<Lazy::LazyRSQ<ll>> segs(N);
   rep(h,N) segs[h] = Lazy::LazyRSQ<ll>(N);
   rep(h,N) rep(w,N) if(P[h][w]=='B'){
      segs[h].set(w, {1,1});
   }
   ...
   rep(q,Q){
      ...
      V<ll> cntH(N);
      rep(h,N) cntH[h] = (D+1+N-h)/N - (U+N-h)/N;
      V<ll> cntW(N);
      rep(w,N) cntW[w] = (R+1+N-w)/N - (L+N-w)/N;
      ...
      rep(h,N) if(cntH[h]>0){
         ll m = (R+1-L)/N;
         ll cand = segs[h].prod(0, N).x*m;
         EL(cand)
         if(L%N<=R%N){
            cand += segs[h].prod(L%N, (R+1)%N).x * cntH[h];
         }
         if(L%N>R%N){
            cand += segs[h].prod(0, (R+N+1)%N).x;            
            cand += segs[h].prod(L%N, N).x;            
         }
...
C 累積和と二分探索
D サンプルが微妙に合わず．
E multiset．なぜ3秒(Python用?)
F セグ木でハッシュ．添え字間違いで3WA．
