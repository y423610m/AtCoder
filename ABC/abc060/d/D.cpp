...
void solve() {
   ...
   ll L = N*3+10;
   V<ll> W(N), Va(N);
   rep(i,N) cin>>W[i]>>Va[i];

   //dp[i][j]:=i番目まででj個選んで，余分な重さがkである
   vector dp(N+1, vector(N+1, vector<ll>(L, -INF)));
   dp[0][0][0] = 0;
   ...
   rep(i,N){
      ...
      rep(j,N){
         rep(k,L){
            if(dp[i][j][k]!=-INF && (long long int)(j+1)*W[0]+k+w<=M){
               chmax(dp[i+1][j+1][k+w], dp[i][j][k]+v);
               chmax(ans, dp[i+1][j+1][k+w]);
            }
         }
      }
      // EL(i)
