...
void solve() {
   ...
   while(!que.empty()){
      ...
      for(const auto& e:G[p]) if(d[e.to]>d[p]+e.cost){
         // auto time = [&](ll ti)->ll {
         //    if(ti+1==0) EL("aa")
         //    return ti + D[e.id]/(ti+1);
         // };
         // auto check = [&](auto x){
         //    // EL(x)
         //    if(time(x)>=time(max(x-1, 0LL))) return true;
         //    return false;
         // };
         // auto dt = binSearch(LINF, t-1, check);
         ll dt = t+D[e.id]/(t+1);
         ll u = sqrt(D[e.id]);
         for(ll a = max(t, u-50);a<u+50;a++){
            chmin(dt, a+D[e.id]/(a+1));
         }
         ll nt = e.cost + dt;
         if(d[e.to]>nt){
            d[e.to] = nt;
