...
void solve() {
   ...
   UpdateUnionFind uf(N);
   auto cmp = [&](int parent, int child)->bool {return parent>=child;};//マージする際にあるべき状態
   auto f = [&](int parent, int child)->void {};//重みに対して行う処理

   rep(i,N){
      rep(j,G[i].size()) if(A[i]>A[G[i][j].to]){
         swap(G[i][j], G[i].back());
         G[i].pop_back();
         j--;
      }
   ...
   rep(i,N){
      for(const auto& e:G[i]) if(A[i]==A[e.to]) uf.merge(i, e.to, cmp, f);
   }

   rep(i,N){
      for(auto& e:G[i]) e.to = uf.leader(e.to);
   }

   rep(i,N) if(uf.leader(i)!=i){
      for(auto& e:G[i]) G[uf.leader(i)].emplace_back(e);
      G[i].clear();
   }

   rep(i,N){
      sort(ALL(G[i]), [&](const auto& a, const auto& b){
         return a.to < b.to;
      });
      G[i].erase(unique(G[i].begin(), G[i].end(), [&](const auto& a, const auto& b){
         return a.to == b.to;
      }), G[i].end());
   }
   ...
   dp[s] = 1;
   min_heap<Tlll> que;
   que.push({A[s], -1, s});//a, cost, p

   while(!que.empty()){
      auto [a, c, p] = que.top(); que.pop();
      c = -c;
      if(dp[p]>c) continue;
      assert(uf.leader(p)==p);

      for(const auto& e:G[p]) if(A[e.to] > A[p]) {
         ll cost = dp[p] + (A[e.to] > A[p]);
         if(dp[e.to] < cost){
            dp[e.to] = cost;
            que.push({A[e.to], -cost, e.to});
         }
      }
   }
