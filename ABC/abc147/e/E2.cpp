...
struct dvector{
    int base;
    vector<T> arr;
    dvector(size_t len, T x, int base):arr(len, x), base(base){}
    // using Tp = typename T;
    using iterator = std::vector<T>::iterator;
    iterator begin(){return arr.begin();}

    T& operator[](int n){return arr[n-base];}
    const T& operator[](int n)const{return arr[n-base];}
};

void solve() {

    //dp[H][W][(H+W)*80]=2* 80**4
