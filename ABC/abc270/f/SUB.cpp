...
void solve() {
    ...
    V<Pii> AB(M);
    V<ll> Z(M);
    rep(i,M) cin>>AB[i]>>Z[i];
    rep(i,M) AB[i].fi--, AB[i].se--;

    ll ans = LINF;

    rep(air,2) rep(port,2){
        ES(air) EL(port)
        Edges<ll> E;
        rep(i,M) E.emplace_back(AB[i].fi, AB[i].se, Z[i], 0, 0);
        if(air) rep(i,N) E.emplace_back(i, N, X[i], 1, 0);
        if(port) rep(i,N) E.emplace_back(i, N+1, Y[i], 1, 0);
        sort(ALL(E), [&](const Edge<ll>& a, const Edge<ll>& b){
            return a.cost<b.cost;
            //???
        });
        ...
        for(auto& e: E){
            if(!tree.same(e.from, e.to)){
                ES(e.from) ES(e.to) EL(e.cost)
                tree.merge(e.from, e.to);
                sum += e.cost;
            }
        }

        if(tree.size(0)>=N) chmin(ans, sum);
        ES(air) ES(port) EL(sum)
    }
    PL(ans)
    return;
}
