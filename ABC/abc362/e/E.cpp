...
void solve() {
   ...
   /*
   dp[i][j][k]:= 最後がi,等比がj, 長さkの数列の数
   */
   V<map<ll, map<ll, mint>>> dp(N);
   
   rep(i,N){
      rep(j, i){
         ll d = A[i] - A[j];
         if(dp[j].contains(d)){
            for(const auto& [len, num]:dp[j][d]){
               dp[i][d][len+1] += num;
            }
         }
         dp[i][d][2] += 1;
   ...
   ans[1] = N;
   rep(i,N){
      for(const auto& [d, mp]:dp[i]) {
         for(const auto& [len, num]: mp){
            ans[len] += num;
         }
      }
   }
   ...
   for(auto [len, num]: dp[N-1][1]){
      ES(len) EL(num)
   }

   repi(i, 1, N+1) PS(ans[i])
   PL("")
