...
template<typename T >ostream &operator<<(ostream &os, const deque< T > &v) {for(int i = 0; i < (int) v.size(); i++) {os << v[i] << (i + 1 != (int) v.size() ? " " : "");}return os;}
...
void solve() {
    ...
    DynamicRollingHash<mint> L(11), R(11);
    // L.A.reserve(N);
    // R.A.reserve(N);
    rep(i,N) L.push_back(S[i]);
    rep(i,N) R.push_back(S[N*2-1-i]);
