...
template <>
struct MinUpdateQuery<std::string,std::string> : public BaseImplicitTreap<std::string, std::string> {
    using BaseImplicitTreap<std::string, std::string>::BaseImplicitTreap;
    MinUpdateQuery() : MinUpdateQuery(numeric_limits<std::string>::max(), numeric_limits<std::string>::min()) {}
    std::string f0(std::string x, std::string y) override {
        if(x=="") return y;
        if(y=="") return x;
        return min(x,y);
    }
    std::string f1(std::string x, std::string y) override { return y == numeric_limits<std::string>::min() ? x : y; }
    std::string g(std::string x, std::string y) override { return y == numeric_limits<std::string>::min() ? x : y; }
    std::string p(std::string x, int len) override { return x; }
};
...
void solve() {
    ...
    TreapRollingHash<mint> rh1(11);//b
    TreapRollingHash<mint> rh2(11);//b

    ll N; cin>>N;
    string S; cin>>S;
    
    rep(i,N) rh1.insert(i, S[i]);
    rep(i,N) rh2.insert(i, S[N*2-1-i]);

    rep(i,N){
        if(rh1.query(0, N)==rh2.query(0,N)){
            string ans;
            rep(i,N){
                ans += char(rh1.query(i,i+1).val());
            }
            PL(ans)
            PL(N-i)
            return;
        ...
        rh1.erase(N-1-i);
        rh1.insert(N-1-i, S[N*2-1-i]);
        rh2.erase(0);
        rh2.insert(N-1, S[N-1-i]);
    }
    PL(-1)
