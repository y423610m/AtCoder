...
void solve() {
    ...
    EditableRollingHash<mint> rh1(N, 11);
    EditableRollingHash<mint> rh2(N, 11);

    rep(i,N) rh1.set(i, S[i]);
    rep(i,N) rh2.set(i, S[N*2-1-i]);

    rep(i,N){
        if(rh1.prod(0,N)==rh2.prod(0,N)){
            PL(i)
            string ans;
            rep(i,N) ans += char(rh1.get(i));
            return;
        }

        rh1.set(N-1-i, S[N*2-1-i]);
        rh2.set()
    }
