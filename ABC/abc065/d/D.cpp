...
void solve() {
   ...
   sort(ALL(id), [&](auto a, auto b){
      return XY[a].fi < XY[b].fi;
   });
   rep(i,N-1){
      que.push({abs(XY[id[i+1]].fi-XY[id[i]].fi), id[i], id[i+1]});
   }
   sort(ALL(id), [&](auto a, auto b){
      return XY[a].se < XY[b].se;
   });
   rep(i,N-1){
      que.push({abs(XY[id[i+1]].se-XY[id[i]].se), id[i], id[i+1]});
   }
