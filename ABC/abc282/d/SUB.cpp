...
void solve() {
    ...
    dsu tree(N);
    rep(i,M) tree.merge(E[i].from, E[i].to);
    V<V<ll>> cnt(N,V<ll>(2));//i(leader)に含まれる0,1色の数


    V<int> color(N,-1);
    queue<Pii> que;//pos, color
    rep(s,N) if(color[s]==-1){
        ...
        while(!que.empty()){
            ...
            for(const auto& e:G[p]){
                if(color[e.to]==-1){
                    color[e.to] = 1-c;
                    cnt[tree.leader(e.to)][1-c]++;
                    que.push({e.to, 1-c});
                }
                else if(color[e.to]==c) END(0)
            }
        }
    }
    ...
    rep(i,N){
        ans += N-tree.size(i);
        ans += cnt[tree.leader(i)][1-color[i]]-G[i].size();
    }
    PL(ans/2)
