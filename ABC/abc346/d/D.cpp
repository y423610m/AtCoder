...
void solve() {
   ...
   //dp[i][j][k]:=i文字目まで見て，最後がjで，連続がk個の最小値
   V<V<V<ll>>> dp(N+1, V<V<ll>>(3, V<ll>(2, LINF)));
   dp[0][2][0] = 0;
   ...
   rep(i,N){
      rep(from,3){
         rep(to, 2){
            ll c = C[i]*((S[i]-'0')==to);
            rep(cnt, 2) if(cnt+(from==to)<=1){
               chmin(dp[i+1][to][cnt+(from==to)], dp[i][from][cnt]+c);
            }
         }
      }
      EL(i)
