...
void solve() {
   ...
   V<ll> S(s.size()), T(t.size());
   rep(i,s.size()) S[i]=s[i]-'a';
   rep(i,t.size()) T[i]=t[i]-'a';

   V<ll> cntS(26), cntT(26);
   V<V<ll>> posS(26), posT(26);
   ...
   auto Find = [&](ll& I, ll& J, ll c, ll k)->bool {
      auto& pos = posT[c];
      ll n = k/pos.size();
      k -= n*pos.size();
      I += n;
   ...
   auto check = [&](auto k){
      ...
      for(auto c:T){
         ll n = cntT[c]*k;
         if(n==0) continue;//Tに含まれてない
         if(cntS[c]*N<n) return false;//そもそも無理

         bool ok = Find(I, J, c, k);
         ES(c) ES(k) ES(I) EL(J)
         if(!ok) return false;
         if(I>N) return false;
      }

      return I*S.size()+J <= N*(S.size());

   };


   ll ans = binSearch(0LL, LINF, check);
   PL(ans)
...
B deque
C setで管理して総和から引く．
D dp[i文字目][末尾がj][連続箇所がk個]. N*2*2．
E 後ろからみる．使った行と列を管理しておく．
F ２分探索．解説と同じことしてそうだけど最後までバグ取れず．
*/