...
void solve() {
    ...
    Edges<ll> E;
    rep(i,N) E.push_back({0,i+1,A[i]});
    rep(i,N) E.push_back({i,(i+1)%(N+1),B[i]});
    ...
    ll ans = 0;
    for(const auto& e:E){
        if(uf.same(e.from, e.to)) ans += e.cost;
        else uf.merge(e.from, e.to);
    }
    PL(ans)
...
void solve2(){
    ...
    //注目する頂点,iの色，1の色，i-1の色
    repi(i,2,N+1) rep(j,2) rep(k,2) rep(l,2){
        ll d = 0;
        if(j==0) d += A[i-1];
        if(j==l) d += B[i-2];
        chmin(dp[i][j][k], dp[i-1][l][k]+d);
    }
