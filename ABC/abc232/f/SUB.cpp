...
void solve() {
   ...
   do{
      ll cand = 0;
      rep(i,N) cand += abs(A[id[i]]-B[i]) * X;

      I = id;
      cnt = merge_sort(ALL(I));
      //cand + cnt*Y > ans
      if(cand>ans) continue;
      if(cnt>(ans-cand)/Y+2) continue;
      cand += cnt * Y;
      chmin(ans, cand);
      ES(cand)
      EL(id)
...
void solve2(){
   ...
   V<bool> used(N, false);
   auto dfs = [&](auto dfs, ll n, ll score)->void {
      if(n==N){
         ES(score) EL(C)
         chmin(ans, score);
         return;
      ...
      // Aのi番目の要素がn番目に来るようにすることを考える
      ll inv = 0;
      rep(i,N){
         if(!used[i]) {
            used[i] = true;
            ll d = X*abs(A[i]-B[n]) + Y*inv;
            C[n] = A[i];
            if(d<ans && score+d<ans) dfs(dfs, n+1, score + d);
            used[i] = false;
            inv++;
         }
      }
...
void solve3(){
   ...
   auto f = [&](ll mask, ll bit)->ll {
      ll ret = 0;
      rep(i,N) if(!(mask&(1<<i)) && i<bit) ret++;
      return ret;
   };
   ...
   rep(mask, 1<<N){
      ...
      rep(bit,N) if(!(mask&(1<<bit))){
         chmin(dp[mask+(1<<bit)], dp[mask]+X*abs(A[bit]-B[cnt])+Y*f(mask, bit));
      }
   }
   PL(dp[(1<<N)-1])
}
