...
void solve() {
    ...
    //dp[i][j]:=i回目のコンテスト見て，jbit見て，最後がkである通り数
    V<V<V<mint>>> dp(N+1, V<V<mint>>(1<<10, V<mint>(11)));
    dp[0][0][10] = 1;
    ...
    rep(i,N){
        ...
        rep(mask,1<<10){
            if(mask&(1<<s)){
                dp[i+1][mask][s] += dp[i][mask][s];
            }
            else{
                rep(k,11){
                    dp[i+1][mask|(1<<s)][s] += dp[i][mask][k];
                }
            }
        }
    }
...
void solve2() {
    ...
    //dp[i][j]:=i回目のコンテスト見て，jbit見て，最後がkである通り数
    V<V<V<mint>>> dp(N+1, V<V<mint>>(1<<10, V<mint>(10)));
    rep(i,10) dp[0][0][i] = 1;
