...
// string kth_element(string& S, ll K){
//     K++;
//     ll N = S.size();
//     string ans;
...
template<typename T = boost::multiprecision::int1024_t>
string kth_element(string& S, T K){
    K++;
    ll N = S.size();
    vector<T> fact_(N+1);
    fact_[0] = 1;
...
void solve() {
    ...
    string S; cin>>S;
    ll K; cin>>K;
    PL(kth_element(S, K-1))
    //rep(i,3) PL(kth_element(S, i))
