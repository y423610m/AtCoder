...
void solve() {
   ...
   V<V<mint>> dp(N+1, V<mint>(st.size()));
   dp[0].back()++;
   ...
   rep(i,N){
      rep(j, st.size()){
         ...
         rep(k,M){
            ...
            if(it==arr.end()){
               if(*prev(it)==k) dp[i+1][j] += dp[i][j];
               continue;
            }
            *it = k;
            auto it2 = lower_bound(ALL(st), arr);
            if(it2==st.end()) EL("error")
            dp[i+1][it2-st.begin()] += dp[i][j];
         }
      }
   }
   ...
   mint ans = 0;
   rep(i,st.size()){
      if(st[i][0]==M) continue;
      if(st[i][1]==M) continue;
      if(st[i][2]==M) continue;
      if(st[i][0]==st[i][1]) continue;
      if(st[i][1]==st[i][2]) continue;
      ans += dp[N][i];
   }
   PL(ans)
