...
struct dijkstra{
    ...
    vector<ll> solve(int s_, int g=-1){
        ...
        dist[s] = 0;
        cnt[s] = 1;
        priority_queue<Pii,vector<Pii>,greater<Pii>> que;//dist, to
        que.push({0,s});
...
void solve() {
    ...
    for(int i=0;i<m;i++){
        ...
        G[u].push_back({u,v,max(H[v]-H[u], 0),i,0});
        G[v].push_back({v,u,max(H[u]-H[v], 0),i,0});
    }
    ...
    rep(i, dist.size()) chmax(ans, H[0]-H[i]-dist[i]);
    PL(ans)
