...
void solve() {
   ...
   rep(q,Q){
      ...
      auto f = [&]()->string {
         if(outer_prod(p-XY[0], XY[1]-XY[0])>0) return "OUT";
         if(outer_prod(XY[N-1]-XY[0], p-XY[0])>0) return "OUT";

         auto check = [&](auto x){
            auto out = outer_prod(p-XY[0], XY[x]-XY[0]);
            ES(x) EL(out)
            if(out<=0) return true;
            return false;
         };

         auto l = binSearch(1LL, N-1, check);
         auto c = outer_prod(XY[l]-p, XY[l+1]-p);
         ES(l) EL(c)
         if(c==0) return "ON";
         if(c>0){
            if(outer_prod(p-XY[0], XY[1]-XY[0])==0) return "ON";
            if(outer_prod(XY[N-1]-XY[0], p-XY[0])==0) return "ON";
            return "IN";
         }
         return "OUT";
      };
