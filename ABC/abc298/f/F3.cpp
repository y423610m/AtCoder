...
void solve() {
   ...
   V<ll> sumR(dictR.size());
   rep(i,N) sumR[R[i]] += X[i];

   V<V<ll>> inR(dictR.size());
   rep(i,N) inR[R[i]].push_back(i);

   RMQ<ll> seg(dictC.size());
   rep(c,dictC.size()) seg.set(c, 0);
   rep(i,N) seg.set(C[i], seg.get(C[i])+X[i]);

   ll ans = 0;
   rep(r,dictR.size()){
      for(auto i:inR[r]){
         seg.set(C[i], seg.get(C[i])-X[i]);
      }
      ES(sumR[r]) EL(seg.prod(0,dictC.size()))
      chmax(ans, sumR[r] + seg.prod(0,dictC.size()));
      for(auto i:inR[r]){
         seg.set(C[i], seg.get(C[i])+X[i]);
      }
   }
