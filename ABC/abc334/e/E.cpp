...
auto inGrid = [&](int nx, int ny){
   ...
   // UndoUnionFind uf(H*W);
   UpdateUnionFind uf(H*W+1);
auto cmp = [&](int parent, int child)->bool {return uf.sz[parent]>=uf.sz[child];};//マージする際にあるべき状態
   ...
   rep(h,H) rep(w,W)if(S[h][w]=='#'){
      rep(d,4){
         ...
         if(inGrid(nx, ny) && S[nx][ny]=='#'){
            uf.merge(h*W+w, nx*W+ny, cmp, f);
            // DSU.merge(h*W+w, nx*W+ny);
         }
      }
   }

   rep(h,H) rep(w,W)if(S[h][w]=='.'){
      uf.merge(h*W+w, H*W, cmp, f);
   }
   ...
   rep(h,H) rep(w,W)if(S[h][w]=='.'){
      ...
      rep(d,4){
         ...
         if(inGrid(nx, ny) && S[nx][ny]=='#'){
            ...
            ll to = nx*W+ny;
            bool ok = true;
            for(auto x:cache) if(uf.same(to, x)) ok = false;
            if(ok){
               cand--;
               cache.push_back(to);
            }
