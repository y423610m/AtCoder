...
void solve() {
   ...
   auto GetDist = [&](const P& a, const P& b){
      return hypot(a.fi-b.fi, a.se-b.se);
   };

   V<ld> dist(N);
   rep(i,N){
      if(i==0) dist[i] = distO[0];
      else{
         dist[i] = GetDist(XY[i], XY[i-1]);
      }
   }

   V<ld> sum(N+1);
   rep(i,N) sum[i+1] = sum[i] + dist[i];

   EL(distO)
   EL(dist)
   EL(sum)

   //dp[i][j]:=i番目にいて，j個持ってる時の最短距離
   // ->NG


   // i個目まで配り終わって家にいるときの最短距離
   V<ld> dp(N+1, LINF);
   dp[0] = 0;
   rep(i,N){
      rep(k,K){
         ll to = i+k;
         if(to>=N) break;
         chmin(dp[to+1], dp[i] + distO[i] + sum[to]-sum[i] + distO[to]);
      }
   }
   EL(dp)
   PL(dp[N]+distO.back())
