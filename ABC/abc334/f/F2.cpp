...
#include "structure/others/swag.hpp"
...
void solve() {
   ...
   ld d = 0;
   d += hypot(sx-X[0], sy-Y[0]);
   rep(i,N-1){
      d += hypot(X[i+1]-X[i], Y[i+1]-Y[i]);
   }
   d += hypot(sx-X[N-1], sy-Y[N-1]);
   
   V<ld> A(N+1);
   rep(i,N-1){
      A[i+1] -= hypot(X[i+1]-X[i], Y[i+1]-Y[i]);
      A[i+1] += hypot(sx-X[i], sy-Y[i]);
      A[i+1] += hypot(X[i+1]-sx, Y[i+1]-sy);
   }
   EL(A)
