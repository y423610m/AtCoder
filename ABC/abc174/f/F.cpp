...
void solve() {
    ...
    sort(ALL(Pos), [&](const array<int,3>& a, const array<int,3>& b){
    ...
    rep(i,Pos.size()){
        if(Pos[i][2]==Q){
            tree.set(Pos[i][1], tree.get(Pos[i][1])+1);
        }
        else{
            cnt[Pos[i][2]] = tree.prod(Pos[i][0], Pos[i][1]+1);
            // cnt[Pos[i][2]] = tree.prod(Pos[i][1]+1, N+2);
        }
    }
