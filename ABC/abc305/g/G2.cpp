...
void solve() {
   ...
   Matrix<mint> A(X.size(), X.size());
   Matrix<mint> x(X.size(), 1);
   string ab = "ab";

   rep(i,X.size()){
      for(auto c:ab){
         string nxt = X[i]+c;
         if(nxt.size()>6) nxt = nxt.substr(1);
         if(binary_search(ALL(X), nxt)){
            int j = lower_bound(ALL(X), nxt) - X.begin();
            A[j][i] = 1;
         }
      }
   }
   ...
   auto y = (A^N) * x;
   mint ans = 0;
   rep(i,y.size()) ans += y[i][0];
   PL(ans)

   // rep(i,y.size()) {ES(i) ES(X[i]) EL(y[i][0])}
