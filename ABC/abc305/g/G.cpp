...
void solve() {
   ...
   */
   ...
   if(N<=6){
      string T;
      auto ok = [&]()->bool {
         rep(i,M) if(T.size()>=S[i].size()){
            bool same = true;
            rep(j,S[i].size()){
               if(T.end()[-1-i]!=S[i].end()[-1-i]) same = false;
            }
            if(same) return false;
         }
         return true;
      };
      auto dfs = [&](auto dfs, ll len)->void {
         EL(T)
         if(len==N){
            ans++;
            return;
...
B Aとほぼ同じ
C #のh,w最大最小求める
D 累積和と二分探索したあと微調整．Moがよぎった
E 警備員のいる頂点からhでダイクストラしつつUF．
F 普通のDFSだけど出力間違えて2ぺナ
G N<=6なら全探索，それ以上は行列累乗と予想
*/