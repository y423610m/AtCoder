...
void solve() {
   ...
   ld inf = LINF;
   V<V<ld>> dp(D+1, V<ld>(1<<N, inf));
   rep(n, D+1) dp[n][0] = powl(mean, 2)*n;
   // rep(i,N) dp[1][1<<i] = pow(W[i]-mean,2);
   rep(mask, 1<<N){
      ld y = 0;
      rep(bit, N) if(mask&(1<<bit)) y += W[bit];
      chmin(dp[1][mask], powl(y-mean, 2));
   }
   
   rep(mask, 1<<N){
      repi(n,1,D){
         chmin(dp[n+1][mask], dp[n][mask] + dp[1][0]);
         ll m = mask;
         while(m>0){
            chmin(dp[n+1][mask], dp[n][mask-m]+dp[1][m]);
            m = (m-1) & mask;
         }
      }
   }
