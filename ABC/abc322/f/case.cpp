...
void solve() {
   ll N = 100;
   LongContinuousBits::LazyRMQ<ll> seg0(N), seg1(N);
   rep(i,N) seg1.set(i, LongContinuousBits::GetS1<ll>());

   rep(i,N) repi(j,i+1,N){
      ES(i) EL(j)
      auto s0 = seg0.prod(i,j);
      auto s1 = seg1.prod(i,j);
      ES(s0)
      EL(s1)
      assert(s0.long0 == s1.long1);
      assert(s0.long1 == s1.long0);
   }
