...
void solve() {
   ...
   rep(is0S, 2){
      rep(is1S, 2){
         //dp[i][j]:=i番目がj(W or S)のとき，辻褄のあう方法あるか
         V<V<bool>> dp(N+2, V<bool>(2));
         V<V<bool>> from(N+2, V<bool>(2));
         dp[0][is0S] = true;
         dp[1][is1S] = true;
         repi(i,1,N+1){
            bool isDiff = S[i%N]=='x';
            rep(wasS, 2) if(dp[i-1][wasS]){
               rep(isS, 2) if(dp[i][isS]){
                  bool willS = wasS ^ isDiff ^ (!isS);
                  if(i%N==0 && isS!=is0S) continue;
                  if(i%N==0 && willS!=is1S) continue;
                  dp[i+1][willS] = true;
                  from[i+1][willS] = isS;
               }
            }
         ...
         rep(i,dp.size()){
            ES(is0S) ES(is1S) ES(i) EL(dp[i])
         }
         if(dp[N][is0S] && dp[N+1][is1S]){
            string S(N, 'W');
            if(is0S) S[0] = 'S';
            if(is1S) S[1] = 'S';
            bool isS = from[N][is0S];
            for(ll i=N-1;i>=2;i--){
               if(isS) S[i] = 'S';
               isS = from[i][isS];
            }
