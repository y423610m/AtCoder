...
void solve() {
    ...
    //dp[i][j]:=i文字使って，長さjであるもr時列の通り数
    V<V<mint>> dp(N+1, V<mint>(N+1));
    // dp[0][0] = 26; dp[0][0] /= 25;
    dp[0][0] = 1;
    rep(i,N){
        rep(from,N+1) if(dp[i][from]!=0) {
            repi(len,1,N+1){
                ll to = from + 1 + sz[len];
                if(i+len<=N&&to<=N){
                    if(from==0) dp[i+len][to] += dp[i][from] * 26;
                    else        dp[i+len][to] += dp[i][from] * 25;
                }
                else break;
            }
        }
...
void solve2(){
    ...
    V<V<mint>> dp(N+1, V<mint>(N+1));
    V<V<mint>> sum(N+1, V<mint>(N+1));

    dp[0][0] = mint(26)/25;
    sum[0][0] = mint(26)/25;

    V<ll> Pow10 = {1, 10, 100, 1000, 10000, 100000};

    rep(i,N+1) dp[0][i] = dp[0][0];
    rep(i,N){
        rep(j,N){
            repi(k,1,4+1){
                if(i-k<0) continue;
                ll r = max(j-Pow10[k-1]+1, 0LL);
                ll l = max(j-Pow10[k]+1, 0LL);

                dp[i+1][j+1] += (sum[i-k][r] - sum[i-k][l]) * 25;
            }
