...
auto inGrid = [&](int nx, int ny){
   ...
   rep(sum, N*2){
      rep(h, N){
         ll w = sum - h;
         if(!inGrid(h, w)) continue;

         set<ll> st;
         rep(i,100) st.insert(i);

         repi(i,1,10){
            if(!inGrid(h+i, w-i*2)) break;
            st.erase(dp[h+i][w-i*2]);
         }
         repi(i,1,10){
            if(!inGrid(h-i*2, w+i)) break;
            st.erase(dp[h-i*2][w+i]);
         }
         dp[h][w] = *st.begin();
      }
   }
