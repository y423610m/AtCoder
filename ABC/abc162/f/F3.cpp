...
void solve() {
   ...
   V<V<ll>> dp(N, V<ll>(2, -LINF));
   dp[0][0] = A[0];
   dp[1][0] = A[1];
   dp[2][1] = A[2];

   rep(i,N){
      if(i-2>=0) chmax(dp[i][0], dp[i-2][0]+A[i]);
      if(i-2>=0) chmax(dp[i][1], dp[i-2][1]+A[i]);
      if(i-3>=0) chmax(dp[i][1], dp[i-3][0]+A[i]);
   }

   ll ans = -LINF;
   rep(i,2) chmax(ans, dp[N-1-i][1]);
   chmax(ans, dp[N-3][0]);
   PL(ans)
