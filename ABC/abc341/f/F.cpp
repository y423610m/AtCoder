...
void solve() {
   ...
   V<ll> W(N); cin>>W;
   V<Pll> sortedW(N);
   rep(i,N) sortedW[i] = {W[i], i};
   V<ll> A(N); cin>>A;

   //無駄な辺を除去
   rep(p,N){
      sort(ALL(G[p]), [&](auto& a, auto& b){
         return W[a.to] < W[b.to];
      });
      while(!G[p].empty() && W[G[p].back().to]>=W[p]) G[p].pop_back();
      ES(p) EL(G[p].size())
   }
   ...
   rep(k,N){
      ...
      //dp[i][j]:=i番目まで使って，WがjであるときのmaxA
      V<V<ll>> dp(G[p].size()+1, V<ll>(W[p], -LINF));
      dp[0][0] = 0;
      V<V<Pll>> from(G[p].size()+1, V<Pll>(W[p], {-1, -1}));
      rep(i,G[p].size()){
         const auto& e = G[p][i];
         dp[i+1] = dp[i];
         from[i+1] = from[i];
         rep(j,W[p]){
            if(j+W[e.to]>=W[p]) break;
            if(dp[i+1][j+W[e.to]] < dp[i][j]+mult[e.to]){
               dp[i+1][j+W[e.to]] = dp[i][j]+mult[e.to];
               from[i+1][j+W[e.to]] = {i, j};
            }
         }
      }
      EL(dp.back())
      ...
      ll MaxIdx = -1;
      rep(j,W[p]){
         if(dp[G[p].size()][j] > Max){
            Max = dp[G[p].size()][j];
            MaxIdx = j;
         }
      }
      mult[p] = dp.back()[MaxIdx]+1;
      ES(Max) EL(MaxIdx)

      auto [a, b] = from[G[p].size()][MaxIdx];
      ES(a) EL(b)
      while(a!=-1){
         nxt[p].push_back(G[p][a].to);
         tie(a,b) = from[a][b];
      }
   }
   ...
   rep(i,N){
      auto [w, p] = sortedW[i];
      ans += A[p];
      ES(ans) ES(A[p]) EL(sortedW[i])
      for(const auto& to:nxt[p]) if(W[to] < W[p]){
         A[to] += A[p];
      }
      A[p] = 0;
      EL(A)
...
B 順番に交換
C やる
D 二分探索．(x/LCM)*2の2忘れに気付かず
E ライブラリ張るだけ->ライブラリのバグと格闘
F Wの小さい頂点からナップサックdp&復元した．これも添え字iを２回使っててバグに気付かず．．
*/