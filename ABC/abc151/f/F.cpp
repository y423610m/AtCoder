...
//ノルム
template<typename T> T norm(pair<T,T> p){return sqrt(p.fi*p.fi+p.se*p.se);}
template<typename T> T norm2(pair<T,T> p){return p.fi*p.fi+p.se*p.se;}
//回転rad
...
void solve() {
    ...
    int N; cin>>N;
    V<pair<double, double>> P(N); cin>>P;
