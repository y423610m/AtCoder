   ...
   rep(q,Q){
      ...
      if(t==1){
         if(st.contains(x)){
            ...
            ll r = st[st.find(x)+1];
            if(r-x<=K) segL.erase(r);
            
            ll l = st[st.find(x)-1];
            if(x-l<=K) segL.erase(x);
         }
         EL(q)
         EL(st)
         EL(segL)
         ...
         EL(x)
         ES(segL.lower_bound(x))
         ES(segL.upper_bound(x))
         auto r = segL[segL.upper_bound(x)];
         auto l = segL[segL.upper_bound(x)-1];
         ES(l) ES(r) ES(st.find(l)) EL(st.find(r))
         PL(st.find(r)-st.find(l))
      }
   }
