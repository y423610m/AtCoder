...
void solve() {
   ...
   rep(mask,1<<N){
      ...
      rep(i,M){
         ll cnt = 0;
         rep(bit,N) if(mask&(1<<bit) && binary_search(ALL(A[i]), bit)) cnt++;  
         if(cnt>=K&&R[i]=='x'){
            ES("ng") ES(i) ES(cnt) ES(1) EL(bitset<15>(mask))
            ok = false;
            break;
         }
         if(cnt<K&&R[i]=='o'){
            ES("ng") ES(i) ES(cnt) ES(2) EL(bitset<15>(mask))
            EL(A[i])
            ok = false;
            break;
         }
