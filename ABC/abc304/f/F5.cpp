...
void solve() {
   ...
   for(auto d:divs){
      ...
      U.push_back(T);
      ll cnt = 0;
      rep(i,T.size()) cnt += T[i] == '.';


      for(string u:U){// u.len > t.len
         if(u.size()==T.size()) continue;
         // if(gcd(u.size(), T.size())==1) continue;
         rep(i,u.size()){
            if(u[i]=='.') T[i%T.size()] = '#';
         }
      }
      ...
      mint cand = 0;
      if(cntd>0) cand = (mint(2).pow(cntd))*(mint(2).pow(cnt-cntd)) -1;
      ans += cand;

      ES(d) ES(T) ES(cntd) ES(cnt) EL(cand)
   }
