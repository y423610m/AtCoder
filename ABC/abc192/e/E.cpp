...
void solve() {
    ...
    for(int i=0;i<m;i++){
        ...
        int k; cin>>k;
        //経路復元はu,v逆!!!
        G[u].push_back({u,v,w,i,k});
        G[v].push_back({v,u,w,i,k});
    }

    dijkstra di(n, G);
    auto dist = di.solve(s);
    //EL(dist)
    if(dist[g]==LINF) END(-1)
    else PL(dist[g])
