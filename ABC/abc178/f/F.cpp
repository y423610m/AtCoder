...
void solve() {
   ...
   rep(i,N) mpA[A[i]]++;
   rep(i,N) mpB[B[i]]++;
   for(auto [a, n]:mpA) if(mpB.contains(a)&&mpB[a]+n>=N) END(No)
   
   ll M = 0;
   for(auto [a, n]:mpA) if(mpB.contains(a)&&mpB[a]+n>=N) END(No)

   for(auto [a, n]:mpA) if(mpB.contains(a)) chmax(M, n+mpB[a]);
   for(auto [b, n]:mpB) if(mpA.contains(b)) chmax(M, n+mpA[b]);
