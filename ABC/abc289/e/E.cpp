...
void solve() {
    ...
    V<V<ll>> G0(N), G1(N);
    for(auto& e:E){
        if(C[e.to]==0) G0[e.from].emplace_back(e.to);
        else G1[e.from].emplace_back(e.to);

        if(C[e.from]==0) G0[e.to].emplace_back(e.from);
        else G1[e.to].emplace_back(e.from);
    }
    

    V<V<ll>> dp(N, V<ll>(N, LINF));
    dp[0][N-1] = 0;
    queue<Pll> que;
    que.push({0, N-1});
    while(!que.empty()){
