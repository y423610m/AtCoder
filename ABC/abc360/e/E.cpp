...
void solve() {
   ...
   ll N,K; cin>>N>>K;
   V<V<mint>> dp(N+1, V<mint>(2));

   mint invNN = mint(1)/(N*N);
   ...
   rep(i,K){
      dp[i+1][LEFT] += dp[i][LEFT]  * (N*N-(N-1))    *invNN;
      dp[i+1][OTHER] += dp[i][LEFT] * (N-1)          *invNN;

      dp[i+1][LEFT] += dp[i][OTHER] * (N-1)          *invNN;
      dp[i+1][OTHER] += dp[i][OTHER]* (N*N-(N-1))    *invNN;
   }

   mint ans = dp[K][LEFT]*1 + dp[K][OTHER]*(2+N)/(N-1);
   PL(ans/N)
