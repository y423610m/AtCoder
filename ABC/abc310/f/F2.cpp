...
void solve2(){
   ...
   V<V<mint>> dp(N+1, V<mint>(1<<11));
   dp[0][1] = 1;
   rep(i,N){
      for(ll j = 0; j < (1<<11);j++){
         // if(dp[i][j]==0) continue;
         // for(ll k=1;k<=A[i];k++){
         for(ll k=1;k <= min(A[i], 10LL);k++){
            
            // if(k>10) break;
            ll to = (j | (j<<k)) & ((1<<11)-1);
            dp[i+1][to] +=  dp[i][j] / A[i];
         }
         if(A[i] > 10){
            dp[i+1][j] += dp[i][j] * (A[i]-10) / A[i];
         }
      }
   }
...
void solve3(){
  ...
  vector<vector<long long>> dp(N + 1, vector<long long>(1 << 11, 0));
  dp[0][1] = 1;
  for (int i = 0; i < N; i++){
    for (int j = 0; j < (1 << 11); j++){
      if (dp[i][j] > 0){
        for (int k = 1; k <= min(A[i], 10); k++){
          int j2 = (j | (j << k)) & ((1 << 11) - 1);
          dp[i + 1][j2] += dp[i][j] * P[i];
          dp[i + 1][j2] %= MOD;
        }
        if (A[i] > 10){
          dp[i + 1][j] += dp[i][j] * (A[i] - 10) % MOD * P[i];
          dp[i + 1][j] %= MOD;
        }
      }
    }
