...
#include "structure/union_find/weight_sum_union_find.hpp"
/*
WeightSumUnionFind<ll> tree(N);
rep(i,N) tree.setWeight(i,A[i]);
if(!tree.same(x,y)) tree.merge(x,y);
tree.leader();
tree.getWeight(x);
abc277 D2
*/
...
void solve() {
    ...
    rep(i,N) if(uf.size(i)!=uf[uf.leader(i)]){
        ES(i) ES(uf.size(i)) EL(uf[uf.leader(i)])
        END("No")
    }
    PL("Yes")
