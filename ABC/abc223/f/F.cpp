...
void solve() {
   ...
   Lazy::LazyRmQ<ll> tree(N+1);
   rep(i,N+1) tree.set(i, {0,1});

   rep(i,N){
      if(S[i]=='(') tree.apply(i+1,N,{1,  1});
      if(S[i]==')') tree.apply(i+1,N,{1, -1});
   }
   rep(i,N+1) ES(tree.prod(i,i+1).x)
   EL("")
   ...
   rep(q,Q){
      ...
      if(t==1){
         ...
         cin>>l>>r;
         l--, r--;
         if(S[l]=='(') tree.apply(l+1,N,{1, -1});
         if(S[l]==')') tree.apply(l+1,N,{1,  1});
         if(S[r]=='(') tree.apply(r+1,N,{1, -1});
         if(S[r]==')') tree.apply(r+1,N,{1,  1});
         swap(S[l], S[r]);
         if(S[l]=='(') tree.apply(l+1,N,{1,  1});
         if(S[l]==')') tree.apply(l+1,N,{1, -1});
         ...
         if(S[l]==')') ok = false;
         if(S[r]=='(') ok = false;
         if((r-l+1)%2!=0) ok = false;
         r++;
         if(tree.prod(l,l+1).x!=tree.prod(r,r+1).x) ok = false;
         if(tree.prod(l,l+1).x>tree.prod(l,r+1).x) ok = false;
         if(ok) PL(Yes)
         else PL(No)
      }
   }
