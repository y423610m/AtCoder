...
//segtree
ll RMQop(ll a, ll b){return max(a,b);} ll RMQe(){return -INF;}
using RMQ = segtree<ll, RMQop,RMQe>;
ll RmQop(ll a, ll b){return min(a,b);} ll RmQe(){return INF;}
using RmQ = segtree<ll, RmQop,RmQe>;// RmQ tree(vec); tree.prod(l,r)
ll RSQop(ll a, ll b){return a+b;} ll RSQe(){return 0LL;}
...
void solve(int tt) {
	...
	auto dfs = [&](auto dfs, int pos, int sum)->void{
		sum += X[pos];
		ans[pos] += sum;
		for(auto to: G[pos]) if(!visited[to]){
			visited[to] = true;
			dfs(dfs, to, sum);
		}
		sum -= X[pos];
