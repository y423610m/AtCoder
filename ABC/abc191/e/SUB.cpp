...
void solve() {
    ...
    int N,M; cin>>N>>M;
    auto E = readE<int>(M, -1, true);
    Graph<int> G(N, E, true);

    V<Dijkstra<int>> Dis(N, Dijkstra<int>(N, INF));
    rep(i,N) Dis[i].solve(G,i);

    V<int> ans(N, INF);
    for(const auto& e:E) if(e.from==e.to) chmin(ans[e.from], e.cost);

    rep(s,N){
        auto& dist = Dis[s].dist;
        rep(to,N) if(s!=to){
            chmin(ans[s], dist[to]+Dis[to].dist[s]);
        }
    }
