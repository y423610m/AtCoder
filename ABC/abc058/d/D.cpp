...
void solve() {
   ...
   Combination<mint> C(max(N, M)+10);

   V<mint> sumH(N-1), sumW(M-1);
   rep(i,N-1){
      sumH[i] = mint(1) * (i+1) * (N-(i+1));
   }
   rep(i,M-1){
      sumW[i] = mint(1) * (i+1) * (M-(i+1));
   }
   EL(sumH)
   EL(sumW)

   rep(i,N-1) sumH[i] *= X[i+1]-X[i];
   rep(i,M-1) sumW[i] *= Y[i+1]-Y[i];
   EL(sumH)
   EL(sumW)

   mint ssH = 0;
   mint ssW = 0;
   rep(i,N-1) ssH += sumH[i];
   rep(i,M-1) ssW += sumW[i];
   EL(ssH)
   EL(ssW)

   mint ans = 0;
   rep(i,N-1) ans += sumH[i] * ssW;
   EL(ans)
   // rep(i,M-1) ans += sumW[i] * ssH;
   // EL(ans)
   PL(ans)
