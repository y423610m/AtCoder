...
using Tlllll = tuple<ll, ll,ll,ll,ll>;
...
auto inGrid = [&](int nx, int ny){
   ...
   vector dist(H, vector(W, vector<ll>(4, LINF)));

   // deque<Tllll> que;//h,w,d, k,
   min_heap<Tlllll> que;//dist, k, h, w, d
   rep(d,4) que.push({0, 0, S.fi, S.se, d});
   rep(d,4) dist[S.fi][S.se][d] = 0;

   while(!que.empty()){
      auto [dis, k, h, w, dir] = que.top();
      k = -k;
      que.pop();
      ES(h) ES(w) ES(k) ES(dir) EL(dist[h][w][dir])
      ...
      rep(d,4){
         ...
         if(inGrid(nx, ny)&&C[nx][ny]=='.'){
            ...
            if(dist[nx][ny][d] > cost ){
               dist[nx][ny][d] = cost;
               if(d==dir){
                  if(k<=0) que.push({cost, -(K-1), nx, ny, d});
                  else que.push({cost, -(k-1), nx, ny, d});
               }
               else{
                  que.push({cost, -(K-1), nx, ny, d});
               }
            }
         }
      }
   ...
   ll ans = LINF;
   rep(d,4) chmin(ans, dist[G.fi][G.se][d]);
   PL(ans==LINF? -1 :ans)

   V<V<ll>> B(H, V<ll>(W, LINF));
   rep(h,H) rep(w,W) rep(d,4) chmin(B[h][w], dist[h][w][d]);
   rep(h,H) EL(B[h])
