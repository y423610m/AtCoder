...
struct State{
    ...
    ll d;
    int num;
    State(int p, ll d, int num):p(p),d(d),num(num){} 
};
...
void solve() {
    ...
    for(auto& e:Es) swap(e.from, e.to);
    ...
    while(!que.empty()){
        ...
        for(auto& e:G[p]){
            ES(p) ES(d) ES(e.to) ES(e.cost) EL(d+e.cost)
            auto it = st[e.id].upper_bound(num);
            if(it==st[e.id].end()) continue;
            if(e.to==0) chmin(dist[e.to], d+e.cost);
            else{
                if(!done[e.to]){
                    chmin(dist[e.to], d+e.cost);
                    que.push(State(e.to, d+e.cost, *it));
                }
            }
        }
    }

    if(dist[0]==LINF) END(-1)
    else END(dist[0])
