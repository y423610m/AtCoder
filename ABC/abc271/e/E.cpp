...
struct State{
    ...
    ll dist;
    int num;
    State(int p, ll dist, int num):p(p),dist(dist),num(num){} 
};
...
void solve() {
    ...
    Edges<ll> Es = readE<ll>(M, -1, true);
    Graph<ll> G(N, Es, true);//directed?

    V<int> E(K); cin>>E;

    V<set<int>> st(M);
    rep(i,K) st[E[i]-1].insert(i);
    ...
    while(!que.empty()){
        ...
        for(auto& e:G[p]){
            auto it = st[e.id].upper_bound(num);
            if(it==st[e.id].end()) continue;
            if(e.to==N-1){
                chmin(ans,dist+e.cost);
                EL(ans)
            }
            else{
                que.push(State(e.to, dist+e.cost, *it));
            }
        }
    }
