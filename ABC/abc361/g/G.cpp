...
void solve() {
   ...
   rep(i,N) rep(d,8){
      ll nx = XY[i].fi+dx[d];
      ll ny = XY[i].se+dy[d];
      auto it = binary_search(ALL(XY), {nx, ny});

      if(it!=XY.end()) G[i].push_back(it-XY.begin());
   }
...
ABC361 6完
C ソートして尺取り
D unordered_map<文字列，距離>でBFS
E 基本どの道も２回通るけど最後と最初を結ぶ経路だけは一回->木の直径分引けばよい
F 降順にbを固定して２分探索．(a^2)^3==(a^3)^2==a^6みたいな重複を排除する
*/