...
void solve() {
   ...
   for(ll b=60;b>=2;b--) {
      ...
      auto check = [&](auto a){
         if(pow(a, b)>N+10) return false;
         if(POW(a, b)<=N) return true;
         return false;
      };

      ll cand = binSearch(1LL, N+1, check);
      // repi(i, 2, cand+1) exa[b].push_back(POW(i, b));
      ans[b] = cand-1;

      repi(c, 2, 60) if(b*c<ans.size()) ans[b] -= ans[b*c];

      ES(b) ES(cand) EL(cand)
      EL(exa[b])
   }
   PL(accumulate(ALL(ans), 0LL))
