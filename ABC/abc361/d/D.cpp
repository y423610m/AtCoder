...
void solve() {
   ...
   mp[S] = 0;
   min_heap<pair<ll, string>> que; que.push({0, S});

   string cache;
   while(!que.empty()) {
      auto [d, s] = que.top(); que.pop();
      if(s==T) END(d)
      ll sid = N;
      rep(i,N) if(s[i]=='.') {
         sid = i;
      ...
      if(sid<0 || N<=sid+1 || s[sid+1]!='.') continue;

      rep(i,N-1) if(s[i]!='.' && s[i+1]!='.' && i!=sid && i!=sid+1 ) {
         cache = s;
         swap(cache[i], cache[sid]);
         swap(cache[i+1], cache[sid+1]);

         if(!mp.contains(cache)){
            mp[cache] = d+1;
            que.push({d+1, cache});
         }
      }
