...
template<typename T, typename tag = rb_tree_tag, typename Compare = less<pair<T, int> > >
struct PBDSMultiset{
    using P = pair<T,int>;
    ...
    void erase(T x){
        ...
        if(id<st.size()){
            auto it = st.find_by_order(id)->fi;
            if(it->fi==x) st.erase(it);
        }
    }
