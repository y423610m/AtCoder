...
void solve() {
   ...
   Combination<mint> C(H*W+10);


   mint ans = 0;

   for(ll h=1;h<=H;h++) for(ll w=1;w<=W;w++){
      if(h*w<K) continue;

      rep(mask, 1<<4){
         ll hh = h;
         ll ww = w;
         rep(dgt, 2) if(mask&(1<<(dgt))) hh--;
         rep(dgt, 2) if(mask&(1<<(dgt+2))) ww--;
         if(hh<0||ww<0) continue;

         mint cand = C.C(hh*ww, K) * (1- ((h-hh+w-ww)%2)*2);
         ans += cand * (H-h+1) * (W-w+1) * (h*w);
      }
   }
