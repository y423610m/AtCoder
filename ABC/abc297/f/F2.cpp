...
void solve() {
   ...
   for(ll h=1;h<=H;h++){
      for(ll w=1;w<=W;w++){
         mint numH = C.C(h*w, K);
         if(w-1>=1) numH -= C.C(h*(w-1), K)*2;
         if(w-2>=1) numH += C.C(h*(w-2), K);
         mint numW = C.C(h*w, K);
         if(h-1>=1) numW -= C.C((h-1)*w, K)*2;
         if(h-2>=1) numW += C.C((h-2)*w, K);
...
A,B やる
C 貪欲に見つけ次第変換
D A>Bとして，AとBの差がBの倍数になるまで，A=A%Bにする．
E 愚直にsetで全通り試す．ただし，すでにAiがsetに含まれているorK個目以上ならcontinue．
たぶん想定解じゃない．
F 包除で行けると思ったけど，できなかった
