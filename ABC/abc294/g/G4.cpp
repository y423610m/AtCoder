...
void solve() {
   ...
   Graph<ll> G(N, E, false);//directed?
   EL(0)
   TreeDistQuery<ll> tdq(N, G, 0);
   EL(1)
   for(auto& e:E){
      tdq.set(e.from, e.to, e.cost);
   }
   EL(2)
   EL(tdq.et.in)
   EL(tdq.et.out)
   ...
   // TreeSumQuery<ll> tree(N, G, 0);
   // rep(i,N) tree.add(i,1);
   // rep(i,N){
   //    ES(i) ES(tree.query(i)) EL(tree.et.numOfChildren[i])
   // }
