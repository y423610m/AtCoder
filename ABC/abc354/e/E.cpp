...
void solve() {
   ...
   V<V<ll>> G(N);
   rep(i,N) repi(j,i+1,N) if(AB[i].fi==AB[j].fi || AB[i].se==AB[j].se) {
      G[i].push_back(j);
   }


   V<ll> dp((1<<N)+2, 0);
   auto dfs = [&](auto dfs, ull state)->void {
      ES(state) EL(popcount(state))
      if(popcount(state)<2) {
         dp[state] = -1;
         return;
      }
      ...
      for(ll j : G[i]) if(state&(1<<j)){
         ull nextState = state ^ (1<<i) ^ (1<<j);
         ES(i) ES(j) EL(nextState)
         if(dp[nextState]==0){
            dfs(dfs, nextState);
         }
         if(dp[nextState]<0) {
