...
void solve() {
   ...
   V<Pll> CV2;
   rep(i,N){
      if(CV2.empty()) CV2.push_back(CV[i]);
      else{
         if(CV2.back().fi==CV[i].fi){
            chmax(CV2.back().se, CV[i].se);
         }
         else{
            CV2.push_back(CV[i]);
         }
   ...
   //dp[i個目まで][k個取り除いた]:N*K
   V<V<ll>> dp(N+1, V<ll>(K+1, -LINF));
   rep(i,N) dp[i][0] = CV[i].se;
   rep(i,N){
      auto [c, v] = CV[i];
      rep(j,K) if(dp[i][j]!=-LINF){//既スキップ数
         rep(k, K+1) {//次skip数
            ES(i) ES(j) EL(k)
            if(j+k>K) break;
            if(i+k>=N) break;
            if(c!=CV[i+k].fi) chmax(dp[i+k+1][j+k], dp[i][j]+CV[i+k].se);
         }
      }
      EL(dp[i+1])
   }
