...
void solve() {
   ...
   set<ll> st;
   rep(i,N-1) if(C[i]==C[i+1]) st.insert(i);

   ll ans = -1;
   rep(i,N) if(i+K<=N) {
      auto itl = st.lower_bound(0);
      auto itr = st.lower_bound(i+K);
      if(itl!=st.end() && *itl<=i-2){
         ES(i) EL(*itl)
         continue;
      }
      if(itr!=st.end()){
      ...
      ll l = i==0? -1 : C[i];
      ll r = i+K-1==N-1? -2 : C[i+K-1];
      ES(i) ES(l) EL(r)
      if(l==r) continue;

      ll sl = seg.prod(0, i);
      ll sr = seg.prod(i+K, N);
      ll cand = sl + sr;
      ES(i) ES(sl) ES(sr) EL(cand);
      chmax(ans, cand);
   }
