...
auto inGrid = [&](int nx, int ny){
   ...
   auto dfs = [&](auto dfs, ll hw)->void {
      ...
      rep(i, N)if(!useT[i]){
         auto [a,b] = AB[i];
         rep(j,2){
            if(inGrid(h+a-1, w+b-1)){
               bool ok = true;
               rep(dh,a) rep(dw,b) if(used[h+dh][w+dw]) ok = false;
               if(ok){
                  useT[i] = true;
                  rep(dh,a) rep(dw,b) used[h+dh][w+dw] = true;   
                  dfs(dfs, hw+1); 
                  useT[i] = false;
                  rep(dh,a) rep(dw,b) used[h+dh][w+dw] = false;   
               }
            }
            swap(a, b);
         }
