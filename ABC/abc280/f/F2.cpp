...
void solve() {
    ...
    for(const auto& e:E){
        if(!wuf.same(e.from, e.to)) wuf.merge(e.from, e.to, e.cost);
        else{
            if(wuf.diff(e.from, e.to)!=e.cost) Infinity[uf.leader(e.from)] = true;
        }
    }
    ...
    rep(q,Q){
        ...
        else if(Infinity[uf.leader(X)]) PL("inf")
        else PL(wuf.diff(X,Y))
    }
