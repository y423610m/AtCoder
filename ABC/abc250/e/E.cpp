...
void solve() {
	...
	map<int,int> posB;
	rep(i,n){
		if(posB.find(B[i])==posB.end()) posB[B[i]] = i;
		else chmin(posB[B[i]],i);
	}
	V<int> okA(n,-1);
	rep(i,n){
		if(i!=0) okA[i] = okA[i-1];
		if(posB.find(A[i])==posB.end()) okA[i] = INF;
		else chmax(okA[i],posB[A[i]]);
	}
	ES("okA") EL(okA)

	map<int,int> posA;
	rep(i,n){
		if(posA.find(A[i])==posA.end()) posA[A[i]] = i;
		else chmin(posA[A[i]],i);
	}
	V<int> okB(n,-1);
	rep(i,n){
		if(i!=0) okB[i] = okB[i-1];
		if(posA.find(B[i])==posA.end()) okB[i] = INF;
		else chmax(okB[i],posA[B[i]]);
	}
	ES("okB") EL(okB)
