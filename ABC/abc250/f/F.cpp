...
void solve() {
	...
	repi(i,2,n){
		P v1;
		v1.x = Ps[i].x-Ps[i-2].x;
		v1.y = Ps[i].y-Ps[i-2].y;
		P v2;
		v2.x = Ps[i].x-Ps[i-1].x;
		v2.y = Ps[i].y-Ps[i-1].y;
		s += abs(v1.x*v2.y-v1.y*v2.x);
		a8 += abs(v1.x*v2.y-v1.y*v2.x);
		b8.push_back(a8*4);
	}
	s*=4;

	ll ans = LINF;
	rep(i,b8.size()) chmin(ans, abs(a8-b8[i]));
	rep(i,b8.size()) chmin(ans, s-abs(a8-b8[i]));
	PL(ans)
