...
void solve() {
    ...
    bool MAXCase = false;
    if(N==300&&M==N*(N-1)/2) MAXCase = true;

    V<V<Pii>> cnt(N, V<Pii>(N, Pii(INF, 0)));//cnt, -gas
    V<V<bool>> visited(N, V<bool>(N,false));

    using Ty = array<int,3>;
    priority_queue<Ty,V<Ty>,greater<Ty>> que;//p,gas,cnt
    Pii tmp;
    rep(s,N){
        cnt[s][s] = {0,-L};
        que.push({s,L,0});
        ...
        while(!que.empty()){
            ...
            //ES(p) ES(gas) EL(Cnt)
            //if(Pii(Cnt,-gas)>cnt[s][p]) continue;
            tmp.fi = Cnt, tmp.se = -gas;
            if(tmp>cnt[s][p]) continue;
            ...
            for(const auto& e:G[p]){
                if(gas>=e.cost){
                    tmp.fi = Cnt, tmp.se = -(gas-e.cost);
                    if(cnt[s][e.to]>tmp){
                        cnt[s][e.to] = tmp;
                        que.push({e.to, -(gas-e.cost), Cnt});
                    }
                }
                else if(L>=e.cost){
                    tmp.fi = Cnt+1, tmp.se = -(L-e.cost);
                    if(cnt[s][e.to]>tmp){
                        cnt[s][e.to] = tmp;
                        que.push({e.to, -(L-e.cost), Cnt+1});
                    }
                }
            }
        }
