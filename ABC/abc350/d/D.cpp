...
// UnionFind uf(N);
// UnionFindUM<ll> uf(N);
// size(x), group(x), Add(x), merge(x, y, w)
...
void solve() {
   ...
   for(auto [a,b]:AB){
      // if(!uf.same(a, b)){
      //    ll leaderA = uf.leader(a);
      //    ll leaderB = uf.leader(b);
      //    uf.merge(a, b);
      //    ll leader = uf.leader(a);
      //    cnt[leader] += cnt[leaderA + leaderB - leader];
      //    cnt[leader]++;
      // }
      // else{
      //    cnt[uf.leader(a)]++;
   ...
   ll ans = 0;
   for(auto g : uf.groups()) if(g.size()>0){
      ll n = g.size();
      ans += n * (n-1) /2;
      // ans -= cnt[g[0]];
   }
