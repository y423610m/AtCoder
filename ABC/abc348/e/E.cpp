...
void solve() {
   ...
   using EdgeT = Pll;//sum, dsum
   using VertexT = Pll;//sum, dsum
   auto Merge = [&](EdgeT a, EdgeT b)->EdgeT {
      return {a.fi+b.fi, a.se+b.se};
   };
   auto El = [&](int p, bool canAffectSelf = false)->EdgeT {
      // if(canAffectSelf) return {0, C[p]};
      return {0, 0};
   };
   auto PE = [&](VertexT v, const Edge<ll>& e)->EdgeT {
      // return {v.fi+v.se, v.se};
      return v;
   };
   auto PV = [&](EdgeT e, int p)->VertexT {
      return {e.fi+e.se, e.se+C[p]};
   };
