...
void solve() {
   ...
   RmQ<ll> segL(N), segR(N);
   rep(i,N) segL.set(i, A[i]-i);
   rep(i,N) segR.set(i, A[i]-(N-1-i));

   ll ans = 0;
   rep(i,N){
      auto check = [&](auto k)->bool {
         k--;
         if(i-k<0 || N<=i+k) return false;

         ll lm = segL.prod(i-k, i+1);
         ll rm = segR.prod(i, i+k+1);
         ES(i) ES(k) ES(lm) ES(rm) EL("")

         if(lm+i-k>0 && rm+(N-1-i)-k>0) return true;
         return false;
      };

      ll k = binSearch(1LL, A[i]+1, check);
      chmax(ans, k);
   }
   PL(ans)
