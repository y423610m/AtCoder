...
void solve() {
   ...
   ll H,W; cin>>H>>W;
   V<V<ll>> S(H, V<ll>(W)); cin>>S; S--;

   ll ans = LINF;

   auto GetHash(vector<vector<ll>>& G, int i)->ull {
      ull h = 0;
      bool ok = true;
      rep(h,H) rep(w,W){
         if(G[h][w]!=h*W+w) ok = false;
         ll g = G[h][w];
         g = g * (h*W+w);
         g = (g << 30 | g) * g;
         h ^= g;
   ...
   auto op = [&](int a){
      int dh = d/2;
      int dw = d&1;
      for(int h=0;h<(H-1+1)/2;h++)
      for(int w=0;w<(W-1+1)/2;w++)
      swap(S[h+dh][w+dw], S[H-1+dh-h][W-1+dw-w]);
   };
...
B while
C 5進数
D 区間minセグ木２つにA[i]-iとA[i]-(N-1-i)．各項から２分探索
E 桁和は高々9*14程度なので，桁和ごとに桁dp. 桁和とそのあまりを持って3次元dp
F ハッシュ持ったら何とかなりそうと思うものの間に合わず
*/