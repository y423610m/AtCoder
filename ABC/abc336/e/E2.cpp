...
void solve() {
   ...
   V<ll> p10 = {1};
   rep(i,17) p10.push_back(p10.back()*10);

   /*
   桁和Sは1~9*14=126までしかない
   14*126
   各桁和について，dp[i][j]:=i桁でj(mod S)である通り数
   
   */

   ll ans = 0;

   for(ll mod=1;mod<=9*L;mod++){
      // dp[isLowerN][keta数][桁和][余り]
      int LOW = 1;
      int SAME = 0;
      vector dp(2, vector(S.size()+1, vector(mod+1, vector<ll>(mod))));
      dp[SAME][0][0][0] = 1;
      rep(i, S.size()){//上からi桁目
         // dp[i+1] = dp[i];
         ll d = S.size() -1 - i;
         rep(j, mod){//余り
            rep(l, mod+1){//桁和
               rep(k, 10) if(l+k<=mod) {//値
                  dp[LOW][i+1][l+k][(j+p10[d]*k)%mod] += dp[LOW][i][l][j];

                  ll v = S[i]-'0';
                  if(v>k){
                     dp[LOW][i+1][l+k][(j+p10[d]*k)%mod] += dp[SAME][i][l][j];
                  }
                  else if(v==k){
                     dp[SAME][i+1][l+k][(j+p10[d]*k)%mod] += dp[SAME][i][l][j];
                  }
               }
            }
         }
         // rep(j, S.size()) ans += dp[LOW][i+1][0] + dp[SAME][i+1][0];
      }
      ll cand = 0;
      ans += dp[LOW][S.size()][mod][0];
      ans += dp[SAME][S.size()][mod][0];
      ES(mod) EL(ans)
      if(mod<=3) rep(i,2){
         rep(j,S.size()+1){
            rep(k,mod+1){
               ES(i) ES(j) EL(dp[i][j][k])
            }
         }
      }
   }
