...
void solve() {
   ...
   */
   ...
   for(ll mod=1;mod<=9*L;mod++){
      // dp[isLowerN][keta数][余り]
      int LOW = 1;
      int SAME = 0;
      vector dp(2, vector(S.size()+1, vector<ll>(mod)));
      dp[SAME][0][0] = 1;
      rep(i, S.size()){//上からi桁目
         ll d = S.size() -1 - i;
         rep(j, mod){
            rep(k, 10){//値
               dp[LOW][i+1][(j+p10[d]*k)%mod] += dp[LOW][i][j];

               ll v = S[d]-'0';
               if(v<k){
                  dp[LOW][i+1][(j+p10[d]*k)%mod] += dp[SAME][i][j];
               }
               else if(v==k){
                  dp[SAME][i+1][(j+p10[d]*k)%mod] += dp[SAME][i][j];
               }
            }
         }
