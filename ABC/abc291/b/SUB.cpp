...
//     vector<typename Iter::value_type> tmp(last-first);
//     Iter l = first, r = mid;
//     Iter p = tmp.begin();
//     while(l<mid&&r<last){
//         if(cmp(*l,*r)) *p++ = *l++;
...
//     while(l<mid) *p++ = *l++;
//     while(r<last) *p++ = *r++;
//     for(Iter it = first; it!=last; it++) *it = tmp[it-first];
// }
...
//     if(first+1<mid) merge_sort(first, mid, cmp);
//     if(mid+1<last) merge_sort(mid, last, cmp);
//     merge(first, mid, last, cmp);
// }
 
// template<typename Iter>
// void merge_sort(Iter first, Iter last){
//     merge_sort(first, last, std::less<typename std::iterator_traits<Iter>::value_type>());
// }
