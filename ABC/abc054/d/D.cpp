...
void solve() {
   ...
   ll N,MA,MB;
   cin>>N>>MA>>MB;
   V<Tlll> ABC(N); cin>>ABC;

   ll n = N/2;
   ll m = N - n;
   ll ans = LINF;

   V<V<ll>> dp(401, V<ll>(401, LINF));
   rep(mask, 1<<n){
      ll A = 0, B = 0, C = 0;
      rep(bit, n)if(mask&(1<<bit)){
         const auto&[a,b,c] = ABC[bit];
         A += a;
         B += b;
         C += c;
      }
      chmin(dp[A][B], C);
      ES(A) ES(B) EL(C)
      if(A*MB==B*MA && B*MA != 0) chmin(ans, C);
   }
   EL(ans)

   rep(mask, 1<<m){
      ll A = 0, B = 0, C = 0;
      rep(bit, m)if(mask&(1<<bit)){
         const auto&[a,b,c] = ABC[bit+n];
         A += a;
         B += b;
         C += c;
      }
      ...
      rep(da, 401){
         ...
         if(db%MA!=0) continue;
         db /= MA;
         if(0<=db && db<=400 &&0<=B+db && B+db<=400 && (B+db)>0){
            chmin(ans, C+dp[da][db]);
         }
      }
   }
