...
void solve() {
   ...
   Edges<ll> E;
   rep(i,N) E.push_back({i, A[i]});
   Graph<ll> G(N, E, true, false);//directed? reverse?
   Graph<ll> invG(N, E, true, true);//directed? reverse?
   ...
   ll ans = 0;
   rep(i,N){
      auto [nearestLoopPoint, d] = pg.nearestLoop[i];
      ans += d;
      ll loopIdx = pg.loopIdx[nearestLoopPoint].fi;
      ES(i) ES(loopIdx) EL(d)
      ans += pg.loops[loopIdx].size();
      // PS(ans)
   }
   PL(ans)
