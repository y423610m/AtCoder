...
/*
    Trie<26, 'a'> tr;//文字数，先頭
    V<ll> id(N);
    rep(i,N) id[i] = tr.insert(S[i]);//挿入．内部配列の何番目かを返す．
    //S[i]と他の最長共通部分列探す
    rep(i,N){
        ll p = id[i];
        while(p!=0){
            if(tr.nodes[p].used>1) break;
            p = tr.nodes[p].parent;
        }
        PL(tr.nodes[p].depth)
    }
...
void solve() {
    ...
    rep(i,N){
        ...
        while(p!=0){
            // if(tr.nodes[p].used>1) break;
            if(tr[p].shared+tr[p].accept.size()>1) break;
            p = tr.nodes[p].parent;
        }
        PL(tr.nodes[p].depth)
    }
