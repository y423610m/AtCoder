...
void solve() {
   ...
   ll H,W,N; cin>>H>>W>>N;
   mf_graph<int> mf(H+W+N*2+2);
   ll s = H+W+N*2;
   ll t = H+W+N*2+1;

   rep(h,H) mf.add_edge(s, h, 1);
   rep(w,W) mf.add_edge(w+H, t, 1);
   ...
   rep(i,N){
      ...
      lw += H;
      uw += H;
      for(ll h=lh;h<=uh;h++) mf.add_edge(h, ln, 1);
      for(ll w=lw;w<=uw;w++) mf.add_edge(rn, w, 1);
   }
