...
void solve() {
    ...
    vector<Dijkstra<int>> Dis(K, Dijkstra<int>(N, INF));
    rep(i,K){
        Dis[i].solve(G, C[i]);
    }
    vector<vector<int>> dist(K);
    rep(i,K) dist[i] = Dis[i].dist;
    dist.emplace_back(vector<int>(N, 1));

    //dp[bit][last]=最小コスト
    V<V<int>> dp(1<<K, vector<int>(K+1, INF));
    dp[0][K] = 0;
    rep(i,1<<K){
        EL(i)
        rep(last, K+1)if(dp[i][last]!=INF){
            rep(to, K)if(!(i&(1<<to))){
                int nxt = C[to];
                chmin(dp[i+(1<<to)][to], dp[i][last]+dist[last][nxt]);
            }
        }
    }
