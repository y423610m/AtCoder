...
vector<ll> KthElementInDigit(ll K, ll Digit){
   vector<ll> result(Digit);
   // if(Digit!=1) 
   result.back() = 1;
   ES(K) ES(Digit) EL(result)
   result[0] += K;
   rep(d, Digit-1){
...
void solve() {
   ...
   repi(d, 0, 50){
      ...
      if(N>p){
         ...
         EL(1)
         auto res = KthElementInDigit(N, d+1);
         V<ll> ans = res;
         reverse(ALL(res));
         repi(i,1,res.size()) ans.push_back(res[i]);
      ...
      if(N>p){
         ...
         V<ll> ans = res;
         reverse(ALL(res));
         repi(i,0,res.size()) ans.push_back(res[i]);
         rep(i,ans.size()) cout<<ans[i];
         END("")
      }
      EL(N)
